...
class VideoDataset(object):
    ...
    def __init__(self, dataset, config, shuffle=True, split=None):
        ...
        subset_test = 'validation' if self.activity_net else 'Test'
    ...
    def split_dataset(self, split=None):
        ...
        elif split == 'Sequence70/30':
            if self.activity_net:
                train_cls = self.classlist[:70]
                test_cls = self.classlist[70:]

        elif split == 'Sequence60/40':
            if self.activity_net:
                train_cls = self.classlist[:60]
                test_cls = self.classlist[40:]
        elif split == 'Sequence50/50':
            if self.activity_net:
                train_cls = self.classlist[:50]
                test_cls = self.classlist[50:]
        elif split == 'random':
            if self.activity_net:
                test_cls = []
                train_cls = random.sample(self.classlist, 80)
                for i in self.classlist:
                    if i not in train_cls:
                        test_cls.append(i)
        else:
            if self.activity_net:
                train_cls = self.classlist[86:]
