...
import torch
from util.function import cat_score, mean_frame, train_sample, test_sample
...
def get_attention_mask_4(mask, samples, batches, args, num_class, comparator=[], mode='train'):
    ...
    samples_flow = samples[:, :, :args.num_dim_cal]
    samples_rgb = samples[:, :, args.num_dim_cal:]

    batches_flow = batches[:, :, :args.num_dim_cal]
    batches_rgb = batches[:, :, args.num_dim_cal:]
    ...
    atten_tsm_rgb, _ = torch.max(tsm_masked_rgb, dim=-1, keepdim=True)
    atten_tsm_flow, _ = torch.max(tsm_masked_flow, dim=-1, keepdim=True)
    '''atten_tsm_rgb = torch.mean(tsm_masked_rgb, dim=-1, keepdim=True)
    atten_tsm_flow = torch.mean(tsm_masked_flow, dim=-1, keepdim=True)'''

    ssm_rgb = get_tsm_cos(samples_rgb, batches_rgb, args, num_class, mode, norm=False)
    ssm_masked_rgb = ssm_rgb * mask
    atten_ssm_rgb, _ = torch.max(ssm_masked_rgb, dim=-1, keepdim=True)
    #atten_ssm_rgb = torch.mean(ssm_masked_rgb, dim=-1, keepdim=True)
    ssm_flow = get_tsm_cos(samples_flow, batches_flow, args, num_class, mode, norm=False)
    ssm_masked_flow = ssm_flow * mask
    atten_ssm_flow, _ = torch.max(ssm_masked_flow, dim=-1, keepdim=True)
    #atten_ssm_flow = torch.mean(ssm_masked_flow, dim=-1, keepdim=True)
    # print(atten_tsm.size(), atten_ssm.size())
...
def train(fcencoder, lr_schedule, optimizer, videodb, epis, args, loss_accumulator, attgen, optimizer_filter, train_class):
    ...
    if args.encoder:
        if args.cuda and torch.cuda.is_available():
            samples = fcencoder(samples, sample_lens, is_query=False)
    ...
    batches = fcencoder(batches, batch_lens, is_query=True)
...
def get_window_feat(feat, window):
    feat_progress = 0
    feat_size = feat.shape[1]
    feat_win = ''
    while feat_progress + window < feat_size:
        if feat_size - feat_progress < window:
            feat_progress = feat_size - window
        feat_w = feat[:, feat_progress: window + feat_progress, :]
        feat_win = cat_score(feat_w.unsqueeze(2), feat_win, 2)
        feat_progress = feat_progress + 1
    return feat_win